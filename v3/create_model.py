...
recipe_full_text_UMAP = reduce_embendings(recipe_full_text_emb,'recipe')
recipe_full_text_PCA = reduce_embeddings_pca(recipe_full_text_emb,'recipe')

training_data = import_training_data() 
training_data = pd.merge(training_data, recipe_full_text_UMAP, on='recipeId', how='left')
training_data = pd.merge(training_data, recipe_full_text_PCA, on='recipeId', how='left')  
...
with open('models/random_forest_recipes_model_us_v3_2k_obs.pkl', 'wb') as f: pickle.dump(model, f)